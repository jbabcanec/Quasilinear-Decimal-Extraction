...
runs in essentially quadratic time or worse. We show that the $N$-th decimal digit of
...
price of $\Theta(N)$ bits of auxiliary space, chiefly the read-only binary
...
$V$ with a division-free construction. Our implementation fits exponent
$N^{1.077}$ across positions $10^{4}$--$10^{7}$ with no upward bend ---
quasi-linear in practice --- while the strongest published decimal
method, Gourdon's, fits $\approx N^{1.8}$ on the same machine and is
outrun $232\times$ already at $N=10^{6}$, with the asymptotic gap widening
without bound. The one remaining logarithm is analyzed as a
barrier with several machine-validated equivalent formulations --- merge depth,
...
Three consequences. First, these faces are \emph{not} constructively
...
$O(N\log^{2}N)$, which in particular is, to our knowledge, the first
...
cost at a single prime, specialized to these arguments) --- and
...
additive face alone. (A complete inventory of the approaches we examined
against that face --- some fifteen routes across real-analytic,
...
maintained in the research dossier accompanying the code.)

\subsection{The additive core, undressed}
...
precisely why the multiplicative faces fall while the additive core does not.
The undressed problem is well charted and open. For an isolated harmonic number
...
converge in law to random processes --- pseudorandom --- even to high
prime-power moduli~\cite{KS16,MZ23}. This is evidence, not proof, but it is the
right neighborhood, and suggests that completeness, rather than the
field-versus-ring distinction, is what renders the complete multiplicative sums
tame.
