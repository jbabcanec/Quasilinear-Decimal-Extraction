...
$N^{1.08}$ across positions $10^{4}$--$10^{7}$ with no upward bend ---
...
outrun $232\times$ already at $N=10^{6}$, with the gap growing without
bound. The one remaining logarithm is analyzed as a
...
single-threaded, CPython 3.10 over GMP). As external baseline we
compiled the verification-grade implementation of Gourdon's
method~\cite{Gourdon03} --- the strongest published decimal extractor
--- and raced it on the same machine; its digits agree with our verified
windows throughout.
