...
\author{Joseph Bab\v{c}anec\\
  \small Department of Mathematics, Benedict College\\
  \small Columbia, SC, USA\\
  \small \texttt{joseph.babcanec@benedict.edu}}
\date{July 2026}
...
\begin{abstract}
The Bailey--Borwein--Plouffe (BBP) formula computes an isolated hexadecimal
digit of $\pi$ without the earlier digits, but no decimal analogue exists:
Borwein, Borwein and Galway proved that $\pi$ has no Machin-type BBP formula
in a base that is not a power of $2$, and every published decimal algorithm
runs in essentially quadratic time. We compute the $N$-th decimal digit of
$\pi$ from the ordinary hexadecimal BBP formula in quasi-linear time, in
$O(N\log^{3}N)$ bit operations and $\Theta(N)$ bits of space. That space is
essentially the binary expansion of one integer, $5^{\,N-1}$, shared by all
terms: after the series is multiplied by $10^{N-1}$, an exact identity
recovers each term's fractional part from residues and bit-windows of the
shared integer. The method works for any base-$2^{r}$ BBP constant, such as
$\ln 2$, gives digits in any base, and repairs the base-5 obstruction of
Zudilin (arXiv:2409.10097). Python and C++ implementations are verified
against public digit corpora through position $10^{7}$, at a measured
exponent of $N^{1.077}$ against $N^{1.8}$ for the strongest published method;
to our knowledge this is the first implemented and verified subquadratic
decimal extractor. It sits one logarithmic factor above the $O(N\log^{2}N)$
cost of computing all $N$ digits by the arithmetic--geometric mean. We
isolate that factor as a single open problem, the computation of a
consolidated $2$-adic integer $V$: its multiplicative part falls to
$O(N\log^{2}N)$ by a classical factorization technique, while the additive
part, equivalent to an isolated harmonic number modulo $2^{N}$, has no known
fast algorithm.
\end{abstract}
...
decimal algorithms form a ladder of essentially quadratic (or worse) methods:
...
\]
bit operations, using $\Theta(N)$ bits of space: the read-only binary
expansion of $5^{\,N-1}$, shared by all terms, together with $O(N)$ bits of
working space (Proposition~\ref{prop:forest}). A space-minimal variant computes
the same digit using only $O(\log^{2} N)$ bits of writable space beyond the
shared string, at the cost of quadratic time. The output is certified in the
Las Vegas sense of Remark~\ref{rem:guard}.
\end{theorem}
...
By the Gauss--Legendre/AGM iteration of Brent and
Salamin~\cite{Brent76,Salamin76}, computing \emph{all} $N$ digits of $\pi$
already takes $O(\mathsf{M}(N)\log N)=O(N\log^{2}N)$ bit operations, and
softly linear time by binary splitting of the Chudnovsky series, both in
$\Theta(N)$ bits of memory; quasi-linear cost per isolated digit is therefore
not by itself a new capability, and the natural benchmark for any extraction
algorithm is \emph{parity with full computation}: matching the
$O(N\log^{2}N)$ of the AGM while retaining the extraction semantics (no
earlier digits produced, a read-only shared string, word-level parallelism).
Our Theorem~\ref{thm:main} sits one logarithmic factor above that benchmark.
Separately, the canonical open
...
(ii) to our knowledge the first \emph{implemented and verified} decimal
extractor of complexity $N^{1+o(1)}$ (softly linear) in any model
...
$\mu2^J$ costs $\Theta(\M(N))$, giving $\Theta(N)$-many big operations. This
is the quadratic wall, and precisely the failure identified
...
costs $O(J_t/w)$ word operations, a quadratic total if done term by
term. It is however batchable
...
$O(\M(L+Tw))$ by superadditivity of $\M$, giving the bound. Naively the
workspace is $O(L+Tw)=\Theta(N\log N)$ bits, held level by level;
Proposition~\ref{prop:forest} reduces it to $\Theta(N)$.
\end{proof}

\begin{proposition}[$\Theta(N)$ space via a remainder forest]\label{prop:forest}
The residues of Theorem~\ref{thm:batch}, and likewise the $V$-phase merge of
Section~\ref{sec:bezout}, can be computed in $\Theta(N)$ bits of working space
while preserving the $O(N\log^{3}N)$ time bound, by processing the $T$ leaves
as a \emph{remainder forest}: partition them into $\Theta(\log N)$ consecutive
blocks of $\Theta(T/\log N)$ leaves each. Each block's modulus product occupies
$\Theta(N)$ bits, and the prefix $X\bmod 2^{J_t}$ that every leaf in a block
requires is a window of the read-only string $X$; the blocks are therefore
independent and are processed sequentially, one resident at a time. Peak
working space is thus $O(N)$ (one block's trees) on top of the read-only~$X$,
and the time is $\Theta(\log N)$ blocks $\times\,O(\M(N)\log N)=O(N\log^{3}N)$,
unchanged up to a constant. This is the space-reduction device of
Harvey--Sutherland (accumulating remainder \emph{forests})~\cite{HS14}.
\end{proposition}
...
$^{\mathrm b}$measured $N^{1.077}$ over $N\in[10^{4},10^{7}]$;
Section~\ref{sec:impl}.}
\end{center}
...
The extractor now runs in four phases. The sieve phase
(Theorem~\ref{thm:sieve}) produces the factorizations, chains, and caches in
$O(N\log\log N)$ word operations. The word phase accumulates all easy terms
and the odd halves of the band terms in fixed point, again in
$O(N\log\log N)$ word operations. The $V$ phase builds $V$ by a balanced
merge of numerator/denominator-product pairs modulo powers of two, using
truncated multiplication at each node and one Newton--Hensel inversion mod
$2^{J_{\max}}$ at the end; it costs $O(\M(N\log N)\log N)$ and is
division-free. (An equivalent variant instead builds the exact sum
$X\cdot(A/B)$, with $A/B=\sum_t\pm1/(\mu_t2^{J_t})$, by binary splitting, at
the same asymptotic cost.) The final phase forms one product
$XV\bmod 2^{J_{\max}}$, reads one window, and outputs the digit, in
$O(\M(N))$. Every logarithm in Theorem~\ref{thm:main} lives in the $V$ phase.
...
$\M(n\log n)\log n$ is an instance of the classical factorial barrier.
Borwein's factorization shaves the \emph{product} to $\M(n\log n)\log\log n$
(Theorem~\ref{thm:crack}); but for a single term of a general holonomic
sequence, which is what $V$ is, no method below the product tree is
known, despite long attention~\cite{Borwein85,Strassen77,BGS07}.

A second formulation: $(\Z/2^{m}\Z)^{*}=\langle-1\rangle\times\langle3\rangle$,
...
tried is $\Theta(N^{2})$ bits. A third formulation is the merge depth itself:
summing $T$ inhomogeneous word-size rationals below $\M(\text{total})\log T$.

A fourth formulation is analytic, and rests on the observation that
...
$w=8/(1+8K)$ (valuation $3$), the lemma says that $E$ (hence
...
Two further formulations matter below. Via the Coleman/Iwasawa
formalism, $E$ is a truncation of the Mazur measure's $\Gamma$-transform;
computing it fast is the same problem as computing the
Kubota--Leopoldt $2$-adic zeta function to high precision, itself open in
computational Iwasawa theory. And most elementary: since
...
forces the formulations above. Two further resources are accounted for: quantum
...
parallelizable in low depth (iterated multiplication and division modulo
$2^{m}$ lie in \textsc{dlogtime}-uniform $\mathsf{TC}^{0}$~\cite{HAB02}), so
at scale it is a cost in work, not in wall-clock time.
\end{remark}

\subsection{A crack: the multiplicative formulations fall}

\begin{theorem}[sub-product-tree evaluation of the multiplicative formulations]
\label{thm:crack}
...
with the direct product, and the measured time ratio grows
monotonically in the factored method's favor.
\end{proof}

Three consequences. First, these formulations are \emph{not} constructively
equivalent: they split into a multiplicative cluster (products, $E$, power
sums, and log-gamma/truncated Kubota--Leopoldt values, now all
$O(N\log^{2}N)$) and the \emph{additive core} $V$ itself, the consolidated
sum $\sum_t\pm2^{\,J_{\max}-J_t}\mu_t^{-1}$, whose numerator admits no prime
factorization. The log-gamma evaluation is, to our knowledge, the first
single-prime \emph{sub-product-tree} evaluation of Diamond's $2$-adic
log-gamma at the arguments $8/(1+8K)$. (Morita's $\Gamma_p$ is implemented in
PARI/GP, Magma and SageMath, PARI/GP's \texttt{lngamma} computes Diamond's
function for $|x|_2>1$, and $\Gamma_p$ is computable in average polynomial
time over all $p\le X$~\cite{CKR20,CKR24}; new here is the sub-product-tree
cost at a single prime, specialized to these arguments.) Second, Theorem~\ref{thm:main} is unchanged, since the
digit pipeline consumes $V$: the barrier now stands, sharper, on the
additive core alone. (A complete inventory of the approaches we examined
against it, some fifteen routes across real-analytic,
$2$-adic, mod-$p$, and circuit-theoretic settings, each with its
terminal obstruction, together with the leads we consider live, is
available from the author.)

\subsection{The additive core as an isolated harmonic number}

\begin{proposition}[the additive core is an isolated harmonic number]
\label{prop:atom}
Let $\Pi(t)=\prod_{k\le K}(8k+1+t)$. Stripping the dyadic weights
$2^{\,J_{\max}-J_t}$ from $V$ leaves the numerator
$\Pi'(0)=\sum_k\prod_{j\ne k}(8j+1)$, and
\[
\frac{\Pi'(0)}{\Pi(0)}=\sum_{k\le K}\frac{1}{8k+1}
\ \equiv\ \tfrac18\Bigl(\psi_2\!\bigl(K+1+\tfrac18\bigr)
-\psi_2\!\bigl(\tfrac18\bigr)\Bigr)\pmod{2^{m}},
\]
a $2$-adic digamma value: equivalently the $x^{1}$-coefficient of a product
of $\Theta(N)$ linear forms, and, via $n!\,H_n=|s(n+1,2)|$, an unsigned
Stirling number of the first kind. Thus, up to the weights, the additive core
is an isolated harmonic number modulo $2^{m}$. (Verified exactly in the
accompanying code.)
\end{proposition}

The product $\Pi(0)$ is \emph{smooth}, so Borwein's factorization applies
(Theorem~\ref{thm:crack}); its logarithmic derivative $\Pi'(0)$ is a generic
integer carrying a large prime factor (already $961$ bits at $K=200$):
\emph{differentiation destroys the factorization the crack runs on}, which is
precisely why the multiplicative formulations fall while the additive core
does not. This problem is well studied and open. For an isolated harmonic number
modulo a prime power, binary splitting~\cite{BZ10} is the best known method; the
factorial's one-logarithm shave is not known to transfer to the sum (posed as
open in~\cite{Johansson09}); and the single-prime cost of $(p-1)!\bmod p^{2}$ is
$\widetilde{O}(\sqrt p)$~\cite{CGH14}, beating which is open and tied to the
absence of polynomial-time point counting~\cite{Harvey14}. The one machine that
carries \emph{both} a product and its sum of partial products (a $p$-adic
$\Gamma$ value and, as we observe, its logarithmic derivative) is the
accumulating remainder tree of Costa--Kedlaya--Roe~\cite{CKR20,CKR24}, but only
amortized over all primes $p\le X$; whether that amortization can be traded for
single-argument precision at the fixed prime $2$ is Open Problem~\ref{op:E}.

The core is also, structurally, an incomplete exponential sum: as a partial sum
of inverses $\sum_{k\le K}(8k+1)^{-1}\bmod 2^{m}$ it sits on the \emph{wild}
side of the tame/wild divide. Quadratic exponential sums are polynomial-time
(Gauss sums), whereas specific degree-$\ge3$ families are $\#\mathrm{P}$-hard
even at a fixed prime power~\cite{CCLL10}; single Kloosterman sums have no known
efficient classical evaluation~\cite{Bruin18}; and partial Kloosterman sums
converge in law to random processes, even to high
prime-power moduli~\cite{KS16,MZ23}. This is evidence rather than proof. It
places the additive core among problems for which no fast algorithm is known,
and suggests that completeness, rather than the field-versus-ring distinction,
is what makes the complete multiplicative sums tame.
...
(Shamir's classical argument~\cite{Shamir79}, in the unit-cost arithmetic model: binary-search
...
\begin{remark}[structural exclusions]\label{rem:exclusions}
Beyond the basis-by-basis evidence, four idea-\emph{classes} for
Open Problem~\ref{op:E} can be excluded outright.
(a) \emph{Formula shopping.} For every BBP family with denominators
$dt+1$ ($d=4,\dots,16$) the truncation-defect of the associated Dwork-type
ratio obeys the same law, $\min_j v_2 = s + v_2(d) - 1$ at truncation depth
$2^{s}$: changing the formula shifts a constant, never the slope, so no
base-$2^{r}$ formula makes the additive core easier. We verified this law
from the Frobenius side, as the valuation of the classical Dwork residual
$f_{s+1}(x)f_{s-1}(x^{2})-f_s(x)f_s(x^{2})$ for
$f={}_2F_1(\tfrac18,1;\tfrac98;x)$; it means Dwork/Frobenius descent supplies
only $\Theta(\log N)$ $2$-adic digits at the depth $s=\Theta(\log N)$ that $N$
terms require, and the one amplifier, an excellent Frobenius lift to modulus
$p^{2s}$, requires $p$ odd~\cite{BV23}, hence is vacuous at $p=2$.
(b) \emph{Depth-doubling (AGM/Landen) iterations.} The duplication law of
the underlying Lerch-type sum,
$\Phi(z,1,a)=\tfrac12\Phi(z^{2},1,\tfrac a2)+\tfrac z2\Phi(z^{2},1,\tfrac{a+1}2)$,
is exact and truncation-compatible but \emph{parameter-branching}: $\log N$
doublings hold $N$ parameters, reproducing the product tree. The classical
logarithm admits an AGM because its transformation fixes a single
parameter; the truncated object branches.
(c) \emph{Incomplete reciprocity.} The real-analytic incomplete
Landsberg--Schaar/theta reciprocity carries an unavoidable transcendental
(Fresnel) remainder~\cite{FK09}; a $2$-adic analogue would need an
algebraic remainder in $\Z/2^{m}$, and a noise-floor-calibrated
lattice-reduction search (linear and quadratic-character $\zeta_{8}$-duals,
two parameter points, planted-relation controls recovered at height $O(1)$)
finds the candidate space empty below the random-lattice threshold.
(d) \emph{Algebraic cross-scale relations in general.} The same
calibrated search over truncated logarithms at five dyadic depths,
their pairwise products, and their exponentials finds no integer relation
between height $O(1)$ (where the known functional equation is recovered
exactly) and the noise floor. Any resolution of
Open Problem~\ref{op:E} must therefore be non-algebraic in the truncation
scale, consistent with Conjecture~\ref{conj:mass}, whose failure, if it
fails, will not be by a scale-recursion.
\end{remark}

\section{Implementation and verification}\label{sec:impl}

The algorithm (the pipeline of
Sections~\ref{sec:method}--\ref{sec:improve}, with the division-free
construction of $V$ realized by symbolic shifts and per-node precision
caps) is implemented in a few hundred lines of Python over GMP
integers (\path{decimal_bbp_extract.py}, covering both $\pi$ and
$\ln2$), accompanied by an independent C++/OpenMP implementation of the
band identity used for cross-checking (\path{three_phase_extract.cpp})
and by standalone exact validators for every identity in the paper
(\path{validation/}). The components cross-validate: the batched band
construction reproduces a direct per-term accumulator bit-for-bit
across the verification battery, and the independent code paths agree
on every digit window reported below.
...
At $N=10^{7}$ it returns $725915133612$ at positions
...
$(K,m)=(100,160)$ and $(257,224)$; the Mazur-measure ($\Gamma$-transform)
form and the extraction $E=\operatorname{Log}(\prod u)/\operatorname{Log}9$
were verified exactly at $96$ bits; and the power-sum congruence was
verified exactly at $(K,r)=(200,120)$ (all $245$ bits), including
recovery of the Log-sum from the elementary power sum alone.

\paragraph{Measurements} (Windows 11 desktop; our implementation
single-threaded, CPython 3.10 over GMP). As external baseline we compiled a
straightforward implementation of Gourdon's method~\cite{Gourdon03}, the
strongest published decimal extractor, faithful to the published algorithm
but not micro-optimized, and raced it on the same machine; its digits agree
with our verified windows throughout. The constant-factor gap below is
therefore only indicative; the claim rests on the \emph{asymptotic} separation,
which follows from the analyses ($N^{2}/\mathrm{polylog}$ for Gourdon versus
$N^{1+o(1)}$ here) and is independent of implementation quality.

\begin{center}
\begin{tabular}{rrrl}
\toprule
$N$ & Gourdon~\cite{Gourdon03} & this paper & auxiliary string\\
\midrule
...
The fitted exponents are $\approx N^{1.8}$ for Gourdon's method
(quadratic-class, as its analysis predicts) against $N^{1.077}$ for
the present algorithm over three decades with no upward bend: the
quasi-linear regime of Theorem~\ref{thm:main} is visible directly, in
interpreted code, and the gap over the strongest published method,
already $232\times$ at $N=10^{6}$, grows without bound. At
$N=10^{7}$ the phase split is $64$ s for the exponentiation loop, $47$ s
for the entire consolidated band (the terms on which every previous
decimal method was quadratic) and $0.05$ s to build $X$. The
sieve-batched exponentiation of Theorem~\ref{thm:sieve} is validated at
the same scale in operation count (identical digits at $N=10^{7}$, with
multiplication counts as predicted); realizing it in wall clock belongs
to compiled implementations, where cost tracks multiplications rather
than interpreter statements.

\section{Open problems}

(1) Engineer a compiled, threaded implementation of the full pipeline
(including the sieve-batched exponentiation, whose operation-count
gains are validated above) for record-scale positions. (2) Open
Problem~\ref{op:E}: the additive core, the one remaining logarithm. (3) Decimal digit extraction in sublinear
...
\paragraph{Use of AI assistance.} Computational experiments, the literature
survey, and manuscript preparation were carried out with assistance from an AI
system (Anthropic Claude). All mathematical identities were independently
machine-validated by the exact-arithmetic scripts in the accompanying
repository, and each externally cited result was checked against its primary
source. The author is solely responsible for the results and their correctness.
...
\bibitem{BBG04}
J.~M.~Borwein, D.~Borwein, W.~F.~Galway,
...
arXiv:2409.10097 (2024). (First arXiv version titled ``\ldots in base 10''.)
...
\bibitem{FK09}
A.~Fedotov, F.~Klopp,
\emph{An exact renormalization formula for Gaussian exponential sums and
applications}, arXiv:0909.3079 (2009).
...
\bibitem{Bernstein04}
D.~J.~Bernstein,
\emph{Scaled remainder trees}, manuscript, 2004-08-20,
...
Mosc.\ J.\ Comb.\ Number Theory \textbf{9} (2020), no.~4, 407--419; arXiv:1912.06345.
...
\bibitem{Johansson09}
F.~Johansson,
\emph{How (not) to compute harmonic numbers}, blog post, 2009,
\url{https://fredrikj.net/blog/2009/02/how-not-to-compute-harmonic-numbers/}.

\bibitem{Harvey14}
D.~Harvey,
\emph{Counting points on hyperelliptic curves in average polynomial time},
Ann.\ of Math.\ \textbf{179} (2014), 783--803.

\bibitem{CCLL10}
J.-Y.~Cai, X.~Chen, R.~J.~Lipton, P.~Lu,
\emph{On tractable exponential sums},
in: FAW 2010, LNCS \textbf{6213} (2010), 148--159.

\bibitem{Bruin18}
P.~Bruin,
\emph{On quantum computation of Kloosterman sums},
arXiv:1807.03600 (2018).

\bibitem{KS16}
E.~Kowalski, W.~F.~Sawin,
\emph{Kloosterman paths and the shape of exponential sums},
Compos.\ Math.\ \textbf{152} (2016), 1489--1516.

\bibitem{MZ23}
D.~Mili\'{c}evi\'{c}, S.~Zhang,
\emph{Distribution of Kloosterman paths to high prime power moduli},
Trans.\ Amer.\ Math.\ Soc.\ Ser.\ B \textbf{10} (2023), 636--669.

\bibitem{CKR20}
E.~Costa, K.~S.~Kedlaya, D.~Roe,
\emph{Hypergeometric $L$-functions in average polynomial time},
in: ANTS-XIV, Open Book Ser.\ \textbf{4} (2020), 143--159.

\bibitem{CKR24}
E.~Costa, K.~S.~Kedlaya, D.~Roe,
\emph{Hypergeometric $L$-functions in average polynomial time, II},
Res.\ Number Theory \textbf{11} (2025), art.\ 32; arXiv:2310.06971.

\bibitem{BV23}
F.~Beukers, M.~Vlasenko,
\emph{Dwork crystals III: from excellent Frobenius lifts towards
supercongruences},
Int.\ Math.\ Res.\ Not.\ IMRN (2023), 20433--20483.
