...
Beyond the basis-by-basis evidence, four broad strategies for
Open Problem~\ref{op:E} can be excluded outright.

The first is formula shopping. For every BBP family with denominators
...
$2^{s}$, so changing the formula shifts a constant but never the slope, and no
...
$p^{2s}$, requires $p$ odd~\cite{BV23} and so is vacuous at $p=2$.

The second is depth-doubling by AGM- or Landen-type iteration. The
duplication law of the underlying Lerch-type sum,
$\Phi(z,1,a)=\tfrac12\Phi(z^{2},1,\tfrac a2)+\tfrac z2\Phi(z^{2},1,\tfrac{a+1}2)$,
is exact and truncation-compatible but branches in its parameter: $\log N$
...
parameter, whereas the truncated object branches.

The third is incomplete reciprocity. The real-analytic incomplete
...
The fourth is algebraic cross-scale relations in general. The same
...
\subsection*{Identity-level validations}
...
These timings are from a Windows 11 desktop, our implementation
single-threaded on CPython 3.10 over GMP. As external baseline we compiled a
...
(1) Open
Problem~\ref{op:E}: the additive core, the one remaining logarithm. (2) Decimal digit extraction in sublinear
...
bits of $5^{\,n}$? (3) The per-term independence invites a distributed
...
exactly this primitive. (4) Zudilin's Gaussian-integer series can
...
Computational experiments, the literature
