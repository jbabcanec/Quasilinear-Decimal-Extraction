...
**SPRINT: intel + the height barrier (2026-07-21 evening).** New theory-results, validated: (1) **H-probe classification**: H = Σ(8k+1)⁻¹ mod 2^m (unweighted V) has EM tail = DERIVATIVE of the cracked log-gamma tail (digamma tail) — identity validated exact 200/200 bits (`h_probe.py`). So V's 2-power weights are NOT the obstruction. (2) **The height barrier**: the Borwein crack evaluates tail_E only at small-height arguments 8/(1+8K), K ∈ ℕ ≤ Θ(N); 2-adic finite-difference differentiation needs neighbors of height 2^{Θ(N)} ⟹ exponential product length ⟹ **the crack cannot be differentiated**. Sharpest sub-target now: digamma-tail values at height-N arguments (falls ⟹ H falls ⟹ only weight-mixing remains between us and V). (3) Jet/dual-number tree computes P′(0) at 3× value-tree cost (no asymptotic gain); factored form carries no jet. Two intel crawlers launched: q-congruence world (V = q-harmonic partial sum at q = 1/16; does creative microscoping evaluate at 2-adic q?) and relaxed/online p-adic + graded-precision (vdH/BvHL relaxed arithmetic; Caruso–Roe–Vaccon precision theory) aimed at the tapered pairing kernel. Board updated.

**THE WIDE BRANCH SWEEP (2026-07-21, late night): FIVE KILLS, ONE MAJOR DISCOVERY, ONE NEW FACE.** Fresh branches, each never tried: (kill 1) randomization/sampling — dead, every term carries unit weight 2-adically, no concentration; (kill 2) Chudnovsky matrix-factorial BSGS — APPLIES via the exact contiguity V(A+8) = 16·V(A) + 1/(A+8) − 16^T/(A−8T+8), but costs Õ(N^1.5) at our aspect ratio (modulus ≈ term count), loses to the tree; (kill 3) Gosper — the term ratio 16(A−8s)/(A−8s−8) is the classical non-summable Lerch case; (kill 4) Mahler/binomial transform — coefficient decay 4k, full length, Pascal-row boundary terms rediscover the mass; (kill 5) inverse-periodicity — 1/μ binary-periodic (period ord_2 mod μ) turns terms into Mersenne folding, O(N) per term, mass again. **DISCOVERY — THE TAME/WILD MIRROR:** finite-field exponential-sum computation has the same dichotomy as our war: TAME character sums (Gauss/Jacobi) = Γ_p products via Gross–Koblitz = cheap ⟺ our cracked multiplicative cluster; WILD sums (Kloosterman Σe_p(x+x̄)) = NO known polylog evaluation (decades open, best ~√p) ⟺ our additive core V. Import program: hunt a reduction V ↔ single-Kloosterman-mod-p (would inherit their decades of failure as hardness evidence). Tested the known exception — Salié sums (quadratic-twist collapses wild to closed form): `twist_hunt.py`, twists of period 2/4/8 + the corrected reflection partner Vr (B = 2(A mod 8) − A, never in any prior basis), two parameter points, controls at height O(1) (including inadvertent duplicate-basis identities found instantly at height 1): **EMPTY at both points — the Salié mechanism does not fire; V is Kloosterman-like all the way down.** **NEW FACE (tenth, the CARRY FACE — from Joseph's "crack it into pieces" push):** slice V's output into G-bit slices: each slice-sum = Σ_t word-computable windows of the periodic strings 1/μ_t (= modexp data, sieve-batchable) — EVERY slice is word-cheap EXCEPT the carry stream between slices: V's entire hardness compresses into Θ(N/G) carry bits. Naive slice algorithm blocked (each term spans J_t/G slices → N²/G word ops — mass conserved), but: (a) the carry face is the BEST face yet for a restricted-model lower bound (carry/communication-complexity techniques actually exist, unlike algebraic-circuit bounds); (b) heuristic Las-Vegas boundary-refinement (compute slices at low precision, refine only near-carry-boundary cases) is open. Piece-audit for the record: smooth-μ subsum cheap but negligible (u^{-u} density); parity/section splits self-similar (no exponent gain); precision-triangle splits gain constants only; slice/carry split isolates the hardness in the thinnest known object (N/G bits).

**ATTACK (2) — L2 SWAP-RECIPROCITY: CLOSED EMPTY (2026-07-21, night). ATTACK (3) — DELIVERED INTO THE PAPER.** L2 test (`l2_swap_hunt.py`): the 2-adic Lerch-FE/Landsberg–Schaar swap — W = Σ_{s<T}16^s/(A−8s) mod 2^P versus 8-term slope-T duals over ℚ₂(ζ₈) (linear ζ₈^k and quadratic ζ₈^{k²} twists), with/without 16^T prefactor, plus interval log — at TWO parameter points (A = 321, 483; controls recovered at height O(1)): every hit dissolved under scrutiny (structural zeros of the quadratic character which only populates components 0,1; 2-power trivia; small-number rational coincidences — verified 2669·31365 = 1845·45373 exactly, the shared factor 17 of 289 and 255). Genuine cross-space EMPTY at both points. Structural insight: at V's true aspect ratio any prefactored dual lives in the top O(G) bits — a swap law would have to be prefactor-free at full weight, which is the configuration tested. Combined with Fedotov–Klopp (the real-world swap REQUIRES the transcendental Fresnel remainder; 2-adically nothing algebraic exists to replace it): L2 closed. Also caught: the "pole-adjacency" framing corrected — A odd makes every denominator a unit; the pole exists only in the continuous parameter; the complete/truncated distinction collapses to self-truncation (consistent with everything). ATTACK (3) deliverable: **Remark (structural exclusions) added to the paper §6** — four idea-classes now excluded in print: (a) formula shopping (cross-formula law, slope universal), (b) depth-doubling/AGM (Lerch duplication is parameter-branching), (c) incomplete reciprocity (Fresnel obstruction + calibrated LLL emptiness, FK09 cited), (d) algebraic cross-scale relations generally (calibrated LLL: nothing between height O(1) and the noise floor). Paper recompiles clean, 15pp. The conjecture's status upgraded: no longer "no idea found" but "these idea-classes excluded." Candidate spin-off noted: the Dwork tower + collapse lemma + engine as a standalone arXiv note ("A Dwork-type congruence for a 2-ramified hypergeometric series").

**ATTACK (1) — THE AGM/DEPTH-DOUBLING ROUTE: CLOSED BY THREE INDEPENDENT KILLS (2026-07-21, evening).** The attack born from the Brent–Salamin reframe (can an iteration double the *truncation depth* M the way Newton doubles precision?) is dead, cleanly: **(Kill 1 — parameter proliferation)**: V's true reflected form recovered from the paper = Σ_{s<T} 16^s/(A−8s) mod 2^{J_max}, A ≈ 2J_max — the truncation edge sits just BEFORE the summand's pole at s = A/8 (why complete-log cheapness never transfers: completion must cross a pole). The Lerch duplication Φ(z,1,a) = ½Φ(z²,1,a/2) + (z/2)Φ(z²,1,(a+1)/2) IS exact and truncation-compatible — but each doubling splits the parameter into two children; after log N levels, N parameters = the product tree in disguise. The log has an AGM because it has ONE parameter; Lerch branches. **(Kill 2 — Bernoulli equivalence)**: the large-parameter route (1/A-expansion: V = (1/A)Σ_j (8/A)^j·Li_{−j}(16), convergent since v(8/A)=3, Li_{−j} = Eulerian rationals — a NINTH face) needs the tapered family of derivation-linked values = exactly the Plouffe-2022 Bernoulli-table mass; the real-analytic analog (fast Hurwitz at large a = EM + Bernoulli tables) says the same. **(Kill 3 — LLL emptiness, `agm_law_hunt.py`)**: all-integer LLL (Cohen 2.6.7) on the baby atom z=6 mod 2^300: controls found the FE at height (1,1,−1) and a planted relation exactly; the hunts — linear cross-scale over odd parts {O_16..O_256, L(6), L(36), 1} (dim 8, noise floor 2^37.5), quadratic/AGM-shape over all products of {T_16,T_32,T_64,L(6)} + linears (dim 18, noise floor 2^16.7), exp-side family (dim 6) — returned ONLY noise-scale artifacts and the trivial 25·25⁻¹=1. Genuine structure appears at height O(1); nothing between O(1) and the noise floor. **The truncated 2-adic logarithm obeys no algebraic relation across dyadic scales — no Landen ladder, no AGM, no depth-doubling. Mass conservation survives its sharpest test.** Next: attack (2) — L2 incomplete Landsberg–Schaar reciprocity (the "crack looks like a reciprocity law" thesis): does Gauss-map/Euclidean descent on incomplete 2-adic quadratic sums keep remainders algebraic in ℤ/2^m (vs the transcendental Fresnel remainder of Fedotov–Klopp over ℝ)? Then attack (3): prove the conjecture.

**STILT AUDIT (2026-07-21, afternoon): PULLING THE LOAD-BEARING ASSUMPTIONS.** User called for questioning foundations, not just adding techniques. Results: **(Stilt 1 — THE UNNAMED BENCHMARK, biggest finding): Brent–Salamin/AGM computes ALL N digits in O(M(N) log N) = O(N log²N), Θ(N) memory — i.e., our campaign's speed target equals what full computation has achieved since 1976.** The prize, correctly stated: EXTRACTION AT PARITY WITH COMPUTATION — same N log²N, keeping extraction semantics (no earlier digits, read-only shared string, word parallelism, spot-verification). Our extraction sits exactly one log above that benchmark. Paper amended ("What is/is not claimed" + Brent76 bibitem); BINGO condition unchanged in substance but its meaning sharpened: V at O(M(N)log N) ⟹ extraction = computation parity. **(Stilt 2 — formula lock-in, pulled and held): cross-formula tower probe (`bellard_tower.py`, families d·t+1 for d = 4,6,8,10,12,16): UNIVERSAL LAW min v₂(R_s) = s + v₂(d) − 1.** Slope 1 per level for EVERY family; the formula only shifts the constant offset (block-alignment bonus v₂(d)); Bellard's unramified 10k-families are worse by constant despite cleaner Frobenius; hex 8t+1 near-optimal. Formula shopping cannot beat the tower ceiling — the atom is formula-independent, now confirmed at the tower level empirically (previously only via the transcendence argument). (Stilt 3 — exp/tail duality noted: exp(trunc-log) = (1−z)^{−1}·(1 − tail + O(tail²)) with tail = complete − trunc and complete cheap; trunc-log ⟺ its own exponential — another exact equivalence, no new cost class.) Standing stilts that survived audit: exactness framing, serial-word model, G = O(log N) guard bits, single-position queries (amortized-precomputation economics don't close the gap at any table size that fits in memory).

**QUEUE #0 CLOSED BY DERIVATION (2026-07-21, 1pm push): THE TOWER'S ENGINE, AND ITS CEILING.** The Dwork tower is now fully understood via two elementary pillars, both validated exact (`tower_engine.py`): (1) **THE COLLAPSE LEMMA**: for i+2k=j, 1/((8i+1)(8k+1)) = [2/(8i+1) + 1/(8k+1)]/(8j+3) — exact algebra (2(8k+1)+(8i+1) = 8j+3) — so every Dwork-ratio coefficient (a convolution of truncated series) COLLAPSES to differences of single harmonic sums over dyadic blocks, scaled by the sibling family 8j+3 (rank-2 mixing explained). Verified to reproduce R_s[j] exactly at all tested (s,j). (2) **THE PRECISION SOURCE**: dyadic-block harmonic sums have closed forms mod 2^{block exponent+3} because inversion permutes complete residue classes (Σv⁻¹ ≡ Σv mod 2^L — verified L = 6..12). Predicted floor v₂ ≥ s+2 matches measurement exactly. **CONSEQUENCE — the ceiling: the tower's O(s) precision IS the complete-block phenomenon, and block sums at deeper precision ARE the H-probe/digamma/truncated-log atom. Amplification of the Dwork tower is EQUIVALENT to the atom — the tower cannot crack V; it is another face of it.** Fifth independent mechanism traced to the same atom (products, EM/log-gamma, Iwasawa/ψ, complete-blocks/Dwork, 8-section/logs): every complete object is cheap, truncation is the sole wall, from every direction. Banked machinery: the collapse lemma (quadratic-in-truncations → linear) has independent algebraic interest. Strategic read: the additive Mass Conservation Conjecture looks increasingly TRUE; the tower + engine + eighth face are paper-grade new mathematics regardless; consolidation recommended.

**STEP 2 OPENS: THE EIGHTH FACE — THE TRUNCATED LOGARITHM (2026-07-21, later still).** Derivation chain: f(x) = Σx^t/(8t+1) is the mod-8 residue filter of −Log(1−y) at y⁸ = x; at x = 16, y = √2 ∈ ℚ₂(ζ₈) (ζ₈+ζ₈⁻¹ = √2, degree-4 ramified field). **8-SECTION IDENTITY VALIDATED (scratchpad `eight_section.py`): Σ_{t≥0}16^t/(8t+1) = (1/(8√2))Σ_{ζ⁸=1} ζ⁻¹(−Log(1−ζ√2)) — verified to 880 2-adic bits in ℚ₂[x]/(x⁴+1), with the three extension components cancelling IDENTICALLY.** Consequence: the COMPLETE 2-adic sum = 8 ramified 2-adic logarithms = quasi-linear (CMTV/Satoh-class over a degree-4 extension). Chasing truncation through the Lerch shift: V = complete (cheap) − truncated-filtered-log. **THE EIGHTH FACE, the canonical atom: compute the truncation to index M of Log₂(1−z) at v(z) = ½, mod 2^m, M ≈ m ≈ N, in Õ(N) ⟺ V ⟺ digit at N log²N.** The complete log is cheap; the truncated log is the entire war, in its purest form — and the Dwork tower of step 1 is now identified as the (1−φψ)-crystal of this filtered logarithm, with U_∞ its unit root (closed-form derivation = next session's exercise, now concrete: the log's functional equation Log(1−z²) = Log(1−z)+Log(1+z) truncates EXACTLY in the unfiltered case; the filter (odd support, mod-8 classes) is what generates the defects — the crystal is the (1−φψ)-projection, tying the Dwork tower to the Iwasawa faces in one object).

**THE DWORK ATTACK, STEP 1 (2026-07-21, late night): THE TOWER EXISTS.** Three empirically-established discoveries on f(x) = Σ x^t/(8t+1) = ₂F₁(1/8,1;9/8;x) at p = 2 — a 2-RAMIFIED case (parameter denominators = 8) not covered by the classical or Beukers–Vlasenko literature (scratchpad `dwork_probe.py`, `dwork_cascade.py`, `dwork_multiplier.py`): (1) **The Dwork congruence holds despite ramification**: F_{2^{s+1}}(x)·F_{2^{s−1}}(x²) ≡ F_{2^s}(x)·F_{2^s}(x²) — coefficientwise EXACTLY ZERO below index 2^s, and mod 2^{s+2} at the boundary; verified at 5 consecutive levels with exact fit v₂ = s+2. Value-level at x=16: trivial tail-agreement 2^{s+2} + genuine Dwork bonus ≈ s+2 bits (fit exact, 8 levels). (2) **Defect structure**: D_s = R_s/2^{s+2} supported on j ≥ 2^s with v₂-profile [0,0,1,1] repeating. (3) **THE CASCADE: ψ(D_{s+1}) = U_s·D_s with U_s CONVERGING 2-adically to a limit multiplier U_∞ at rate exactly 2^{−s}** (v₂(U_{s+1}−U_s) gains 1 per level, every coefficient; tail coefficients already stable mod 256: [·,·,208,208,80,80]). This is textbook Frobenius/unit-root tower structure — the iterability precondition for BV-III-style precision amplification — discovered empirically in a ramified case with an explicitly computable multiplier. **Status: the Dwork door is wide open; the amplification question (O(s) → Θ(N) precision) is now a concrete study of THIS tower's iterates. The prize remains open.** Next: (a) derive U_∞ in closed form (candidate: unit root of the ₂F₁ connection's Frobenius at x=16); (b) iterate the two-level structure and measure composite precision; (c) attempt the BV-III excellent-lift upgrade on our tower.

**SPRINT CRAWLER RETURNS (2026-07-21 night): the Dwork lead + four doors closed with reasons.** Crawler 1 (relaxed/tapered): block-pairing route DISPOSITIONED at Θ(N^{1.5}) (C-finite coefficient-jump bound; every block size loses to our tree); "precision-graded inner product" verified genuinely unstudied as a named problem; Giorgi et al. 2402.10139 = the total-mass benchmark (Õ(s log D)) proving never-materialize is necessary; relaxed p-adic (vdH/BvHL) = right model, no graded theorem; CRV precision theory = correctness not asymptotics; caution logged: BLS-Tellegen has NO mixed-precision content (automated summary claimed it; verified false). Crawler 2 (q-world): **★ THE DWORK/CARTIER LEAD ★** — Beukers–Vlasenko Dwork Crystals I–III / Vlasenko p-linear schemes / CKR amortized HGM: Cartier gives EXACT truncation-p^{s+1}-to-p^s congruences — the ONLY truncation-relating exact mechanism in the literature — and our f(x) = Σ x^t/(8t+1) is Dwork-admissible at p = 2 (denominators are 2-units) with x = 16 in the disk. Gap precisely drawn: classical descent carries O(log T) precision, we need Θ(N); amplification unstudied AND unruled-out; CKR II (transcendental corrections in descent) = working precedent. New queue #0: derive Cartier structure for our series, validate the classical congruence numerically, then attack precision amplification via excellent Frobenius lifts. Doors closed with citable reasons: Gosper/telescoping closed form PROVABLY impossible (mixed-class summability 2602.03289); q-supercongruence school no-transfer (moduli specialize odd, info = jets at roots of unity); weighted-Dedekind descent survives ONLY finite-order twists (infinite-order geometric weights move the modular variable — structural explanation for our empty linear stratum!); q-Volkenborn out of domain (|1−q|₂ < 1 required). Lerch completion identity holds 2-adically (complete sums converge) but complete sums cost their terms absent quadrature — consistent with Volkenborn rigidity.

**THE FINAL-NAIL SESSION (2026-07-21, experiments run).** (1) **Block-anchor reduction VALIDATED EXACT** (`block_anchor.py`: 174 j-terms, 512 bits) — the tapered-pairing kernel formulation is sound; the minimal open problem stands as stated. (2) **LLL fingerprint hunt executed** (`relation_hunt.py`): lattice-mining S(K) data mod 2^{2r+5} across (r,K) instances with persistence filtering. **Methodology validated: the hunt REDISCOVERED the seventh-face theorem from raw data** (S − 2^r·Log P − K = 0, persistent at all instances, plus scale-combinations) after fixing a component bug (must feed 2^r·Log₂P, not 2^r·dlog₉P — odd-factor invisible to small-coeff LLL). **Pure-S linear stratum: EMPTY** — zero persistent relations among {S at scales 1,2,4; boundaries; K; 1}; single-instance artifacts correctly rejected. Conclusion: no Dedekind-style LINEAR descent with coefficients ≤ 2^14 exists among these natural components at these scales. The unhunted frontier for Route B: the DUAL/bilinear stratum — defining the parameter-swapped reciprocal object (the L–S analogue swaps modulus↔multiplier; our (K, r) dual is undefined) and hunting products/bilinear combinations. Routes table +1 row in paper.

**THE REMAINING TASK, DISTILLED (2026-07-21, derivation — validation queued).** Post-crack, the digit needs exactly: Num = Σ_t ±2^{Jmax−J_t}·Π_{s≠t}μ_s mod 2^{Jmax} in O(M(N) log N) [denominator product + inverse now free]. **New reduction (block anchoring):** within a block anchored at μ₀, expand μ^{−1} = μ₀^{−1}Σ(−8τμ₀^{−1})^j 2-adically; interchange sums ⟹ per-family V = Σ_i w_i·W(z_i) where W(z) = Σ_{τ<b} 2^{−4τ}/(1−zτ) is ONE FIXED small rational function (degree-b, coefficients only O(j log b) bits — total description Õ(b²log b), Õ(N) at b=√N) evaluated at N/b block points z_i = −8/μ₀(i), v₂(z_i)=3. In coefficient space: V-family = Σ_j f_j·p_j — a **TAPERED BILINEAR PAIRING**: f = small-generated coefficient vector (from a degree-b product), p_j = Σ_i w_i z_i^j = weighted power sums of N/b points, each needed mod 2^{m−3j}. AND the p-side is itself a smaller instance of the same shape ⟹ **recursion: if the pairing kernel is Õ(N), the whole additive core collapses to Õ(N polylog) and the digit lands at N log²**. The pairing of two tapered mass-m²/6 vectors is the irreducible kernel — smaller and sharper than V itself. THE THREE ROUTES: (A) crack the tapered pairing (never materialize either vector — both are small-generated; this is the new minimal open problem); (B) incomplete 2-adic reciprocity hunt — EXPERIMENT: compute remainders R(K) = S(K) − reciprocity-main-term numerically, test for low-complexity structure (Dedekind-paradigm); (C) holonomic completion identity (interval indicator via holonomic kernels — low odds); (D) or prove the additive Mass Conservation Conjecture. All V-attacks may now use products/E/log-gamma as free subroutines.
...
### 16.10 Audit, Dwork closure, and arXiv hardening (2026-07-21, later session)

**Foundation re-verified live.** The reference implementation was re-run cold: continuous positions 1–60 (0 errors), spot checks through 10⁴, the Strassnitzky–Dase independent-formula cross-check, ln 2, and the v3 division-free pipeline all pass; measured slope N^1.065 over 10⁴–10⁶, and the 10⁶ digit **130927562832** reproduces. The reduction to V was re-derived from the code up and confirmed: V is a truncated 2-adic logarithm with a uniformizer argument, sitting one log-factor above M(N) log N because the product-tree working mass is Θ(N log N), not Θ(N).

**Dwork excellent-lift descent — CLOSED (the campaign's top live algorithmic hope).** `validation/dwork_amplify.py` + `dwork_crossformula.py`: the classical Dwork residual f_{s+1}(x)·f_{s−1}(x²) − f_s(x)·f_s(x²) for f = ₂F₁(1/8, 1; 9/8; x) at p = 2 has minimum coefficient valuation **exactly s + v₂(d) − 1** across d = 2, 4, 8, 16 — an independent construction reproducing the cross-formula tower law (slope 1, formula-independent). So basic Frobenius descent supplies only Θ(log N) 2-adic digits at the depth log N that N terms require. The one amplifier in the literature — an excellent Frobenius lift to modulus p^{2s} (Beukers–Vlasenko, *Dwork Crystals III*) — requires p odd (the refinement needs 1 ≤ k < p) and is **vacuous at p = 2**; Beukers–Vlasenko could not prove even the p^{2s} truncated-sum congruence. Amplification is not available by this route. (The standalone Dwork-tower note remains worth writing as a self-contained congruence result; only the "it cracks V" hope is dead.)

**The atom is a named open problem (five-agent literature sweep, primary sources read).** Stripping the dyadic weights, V's core is the numerator Π′(0) of Π(t) = ∏_{k≤K}(8k+1+t), and Π′(0)/Π(0) = Σ_{k≤K} 1/(8k+1) mod 2^m = a 2-adic digamma value = (via n! H_n = |s(n+1,2)|) an unsigned Stirling number of the first kind (`validation/atom_check.py`: Π(0) smooth, Π′(0) generic with a 961-bit prime factor at K = 200 — differentiation destroys the factorization the Borwein crack runs on). Computing an isolated harmonic number mod 2^N below the product-tree bound O(N log³N) is OPEN and coupled to the factorial-mod-prime-power barrier: Johansson (FLINT/Arb) reports nothing beats binary splitting for an isolated H_n and the factorial log-shave is not known to transfer to the sum; Harvey's single-prime (p−1)! mod p² is Õ(√p), beating it open and tied to no-poly-time point counting, with Kurepa's Σk! the identical 2×2 accumulator. Hardness neighborhood (evidence, not proof): Cai–Chen–Lipton–Lu #P-dichotomy (quadratic exp-sums in P via Gauss sums, degree ≥ 3 #P-hard even at fixed prime power); Bruin (no efficient classical single Kloosterman sum); Kowalski–Sawin and Milićević–Zhang (partial Kloosterman sums pseudorandom even mod p^k). It is the arithmetic-mod-2^N twin of "compute π or e to N bits in O(M(N)) without the binary-splitting log factor." Closest existing machine: Costa–Kedlaya–Roe `amortizedHGM` carries a product and its sum-of-partial-products in one block-triangular matrix (a p-adic Γ and — our observation — its digamma), amortized over all primes p ≤ X; trading that for single-argument precision at fixed p = 2 is the sharp open question (likely a long shot — amortization is the speedup).

**Paper hardened to arXiv-ready** (`paper_audit.md` = full ledger). Two blockers fixed: (i) the main theorem claimed Θ(N) space *and* quasi-linear time simultaneously, which no single variant delivers (v1 is Θ(N) space at O(N²) time; the batched tree is quasi-linear at Θ(N log N) space) — now earned honestly via a **remainder-forest Proposition** (Θ(N) space at quasi-linear time, Harvey–Sutherland); (ii) the false claim "no p-adic Gamma routine exists in the standard libraries" — PARI/GP's `lngamma` computes Diamond's log-Γ for |x| > 1, exactly our arguments. Five majors (log-Γ novelty re-anchored on CKR; Gourdon comparison fairness; "subquadratic" → N^{1+o(1)}; ZZ15 wrong venue → *Mosc. J. Comb. Number Theory* 9 (2020); BBG04 author order → J. M. Borwein first) and polish (HAB02 wired into a TC⁰ claim, KL24 orphan removed, Brent/Salamin bibitem split). §6 fortified with all the literature above; every one of ~33 citations verified against its primary source; AI-assistance disclosure added. Result: 37 references, 0 orphans, 0 dangling refs; compiles clean at 17 pp.

**Live fronts now:** (1) prove mass conservation in a restricted model via the carry face (communication-complexity flavor — the exclusions remark is scaffolding); (2) the Kloosterman↔V reduction hunt (import the tame/wild hardness evidence); (3) the CKR single-argument question (lower probability).
